(function (PMV) {
  ...
  // Soma luz direta a uma cor (termo aditivo, não mistura). A diferença
  // importa: misturar com lerp em direção ao branco LAVA a cor (a areia
  // vira cinza claro); somar CLAREIA mantendo o matiz, que é como luz real
  // se comporta. Usado pro termo de luz direta do modelo de iluminação -
  // ver shadeTerrain em themes/recife/background.js.
  CanvasUtils.addHexLight = function (hex, lightHex, amount) {
    if (amount <= 0) return hex;
    hex = hex.replace('#', '');
    lightHex = lightHex.replace('#', '');
    function ch(s, i) { return parseInt(s.substring(i, i + 2), 16); }
    function h(n) { n = CanvasUtils.clamp(Math.round(n), 0, 255); var s = n.toString(16); return s.length === 1 ? '0' + s : s; }
    return '#' +
      h(ch(hex, 0) + ch(lightHex, 0) * amount) +
      h(ch(hex, 2) + ch(lightHex, 2) * amount) +
      h(ch(hex, 4) + ch(lightHex, 4) * amount);
  };

  // Quanto uma superfície está virada pra luz, em [0,1]. (dx,dy) é a
  // tangente local da curva; lightDirX inclina a luz na horizontal (0 =
  // vindo direto de cima). Em canvas o y cresce pra baixo, então a normal
  // "pra cima" de uma tangente (dx,dy) é (dy,-dx).
  CanvasUtils.facingLight = function (dx, dy, lightDirX) {
    var nLen = Math.sqrt(dx * dx + dy * dy) || 1;
    var nx = dy / nLen, ny = -dx / nLen;
    var lx = lightDirX || 0, ly = 1;
    var lLen = Math.sqrt(lx * lx + ly * ly) || 1;
    return CanvasUtils.clamp(-(nx * lx + ny * ly) / lLen, 0, 1);
  };

  CanvasUtils.hexToRgb = function (hex) {
    hex = hex.replace('#', '');
    return {
